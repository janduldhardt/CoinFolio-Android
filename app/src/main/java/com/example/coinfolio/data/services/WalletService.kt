...
class WalletService {
    ...
    fun uploadTransaction(transactionDTO: TransactionDTO) {
        ...
        val date = Converters().zonedDateTimeToString(transactionDTO.zonedDateTime)
    ...
    fun addValueEventListenerFirebase(walletId: String, transactionDao: TransactionDao) {
        val walletListener = object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                val dataStringWalletWithTransactions =
                    dataSnapshot.child("wallets")
                        .child(walletId)
                        .child("transactions")
                        .value ?: return

                val transactionsHashMap = dataStringWalletWithTransactions as HashMap<*,*>
